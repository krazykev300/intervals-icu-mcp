...
This fork defaults to `INTERVALS_ICU_TOOL_PROFILE=coaching` (34 tools: calendar, fitness, athlete-state, ATP read, curves, wellness, activities, streams/histograms, workout library) plus 6 MCP Resources and 10 MCP Prompts. Set `INTERVALS_ICU_TOOL_PROFILE=full` for the complete catalog (up to 64 tools). See [Available Tools](#available-tools) and [docs/tools.md](docs/tools.md).
...
64 tools exist in the `full` profile; this fork's default `coaching` profile registers 34. One-line summary below — full reference in [docs/tools.md](https://github.com/hhopke/intervals-icu-mcp/blob/main/docs/tools.md).
...
| [Athlete](https://github.com/hhopke/intervals-icu-mcp/blob/main/docs/tools.md#athlete-5-tools) | 5 | Profile, CTL/ATL/TSB, fitness chart, and composite athlete-state grounding |
